    ...
    if (typeof io !== "undefined") {
      ...
      this._socket.on("connect", function () {
        if (that._disconnectedSince) {
          ...
          if (
            that._reloadInterval &&
            offlineTime > that._reloadInterval * 1000 &&
            !that.authError
          ) {
            that.reload();
          }
      ...
      this._socket.on("objectChange", function (id, obj) {
        ...
        that._connCallbacks.onObjectChange &&
    ...
    this._socket.emit(
      ...
      function (err, res) {
        ...
        that.getStates(_hosts, function (err, states) {
          for (var h in states) {
            if (
              states.hasOwnProperty(h) &&
              (states[h].val === "true" || states[h].val === true)
            ) {
